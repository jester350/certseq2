function readdb () {
    ...
    return new Promise (function (resolve, reject){pool.query('SELECT cert.row_id as rowid, cert.name as certname,cert.issue_date as certissueDate,cert.expiry_date as certExpiryDate,systems.name as systemName, cert.issue_date+99 as testdata \
    ...
    inner join systems on systems.row_id = cert_system_junc.system ORDER BY certExpiryDate ASC', (err, res) => {
            ...
            for (var i in res.rows) {
                ...
                sdate=date.format(res.rows[i].certissueDate,'DD-MM-YYYY');
                res.rows[i].certissueDate = sdate;
